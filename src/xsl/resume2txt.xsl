...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:resume="http://ns.hr-xml.org/2006-02-28" exclude-result-prefixes="resume">
	...
	<xsl:template match="resume:Associations">
		<xsl:call-template name="getHeading">
			<xsl:with-param name="text" select="'PROFESSIONAL ASSOCIATIONS'" />
		</xsl:call-template>
	...
	<xsl:template match="resume:PatentHistory">
		<xsl:call-template name="getHeading">
			<xsl:with-param name="text" select="'SOFTWARE PATENTS'" />
		</xsl:call-template>
